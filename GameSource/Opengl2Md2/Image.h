...
class DllExport ImageException : public std::runtime_error
{
public:
	// Constructors
...
class DllExport ImageTGA : public Image
{
public:
	// Constructor
...
class DllExport ImagePCX : public Image
{
public:
	// Constructor
...
/////////////////////////////////////////////////////////////////////////////
//
// class ImageFreeImage - FreeImageLib
//
/////////////////////////////////////////////////////////////////////////////

class DllExport ImageFreeImage : public Image
{
public:
	// Constructor
	FIBITMAP *dib;

	ImageFreeImage (const ImageBuffer &ibuff);
...
class DllExport ImageColortoAlpha : public Image
{
public:
...
public:
	...
	static Image *createImage (const ImageBuffer &ibuff)
	{
		...
		{
			result = new ImageFreeImage(ibuff);
		}
	...
	// Public interface
	static Image *createImage (const ImageBuffer &ibuff,const ImageBuffer &aibuff)
	{
		string ext;
		string alphaext;
		Image *result;
		...
		// Extract file extension
		const string &alphafilename = aibuff.filename ();
		alphaext.assign (alphafilename, alphafilename.find_last_of ('.') + 1, string::npos);

		//TextureManager::Inst()->LoadTexture(

		
		if (ext.compare ("pcx") == 0 && alphaext.compare ("pcx") == 0)
		{
			result = new ImageColortoAlpha (ibuff,aibuff);
		}
		else if (ext.compare ("PCX") == 0 && alphaext.compare ("PCX") == 0)
		{
			result = new ImageColortoAlpha (ibuff,aibuff);
		}
		else
