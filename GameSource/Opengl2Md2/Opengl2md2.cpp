...
void	Opengl2md2::begin2D ()
{
	...
	glLoadIdentity ();
	glOrtho (0, 340, 0, 400, -1.0f, 10000.0f);
...
void Opengl2md2::ProcessSelect(GLuint index[128])  // NEW //
{
	...
	if (inst->Close2d)
	{
		...
		if (SelectList.size() == 0)
		{
			...
			if (GameWorld::getInstance().Volkes != NULL)
				GameWorld::getInstance().Volkes->SetMd2ObjectSelection(NULL);
		}
		else
		{
			// 맨 위에 객체만 선택한다. 
			...
			if (ObjectMove::getinstance()->SelectObjectNum.size() != 0)
			{
				obj = (Md2Object*)GameWorld::getInstance().FindbyNameObj(
			...
			if (obj != NULL)
			{
				...
				if (GameWorld::getInstance().Volkes != NULL)
					GameWorld::getInstance().Volkes->SetMd2ObjectSelection(obj);
	...
	else
	{
		...
		//	if (GameWorld::getInstance().Volkes != NULL)
		//		GameWorld::getInstance().Volkes->SetImageControlSelection(node);
		//
		//
		//	
		//	//inst->render->onDrawFrame();
				...
				if (obj != NULL)
				{
					if (inst->emTrancelate == EM_TRANCELATE)
					{
						...
						for (int i = 0; i < ObjectMove::getinstance()->SelectObjectNum.size(); i++)
						{
							...
							SelectableObject* node = dynamic_cast<SelectableObject*>(ObjectNumberingMananger::getInstance()->GetGameObejct(
								ObjectMove::getinstance()->SelectObjectNum[i]));
							if (node != NULL)
								node->Dragging(mouse);
						}
						
						GameWorld::getInstance().Volkes->SelectedObjectChanged();
					}
...
{
	ExGameGraphicInfo::GetGameGraphic()->SetGameSize(Point(1280, 720));
	// Initialize GLUT
	glutInit(&argc, argv);
...
void Opengl2md2::Load(string sModelname)
{
	if (inst->Close2d)
	{
		GameWorld::getInstance().Load(sModelname);
	}
	else
	{
