...
Md2Model::Md2Model (const string &filename)
	...
	_glcmds (NULL), GameObejct(MARX_OBJECT_MD2_MODEL)
{
	_scale =1.0f;
	_tex  = NULL;

	string AbsolutePath = GameWorld::getInstance()._RootDirctory + "/"+ filename;
	// Open the file
...
bool Md2Model::loadTexture (const string &filename , const string &alphafilename)
{
	// Get texture manager
	Texture2DManager *texMgr = Texture2DManager::getInstance ();
...
bool Md2Model::setTexture (const string &filename)
{
...
bool Md2Model::setTexture (const string &filename, const string &alphafilename)
{
	SkinMap::iterator itor;
