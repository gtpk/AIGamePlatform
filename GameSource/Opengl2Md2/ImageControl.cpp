...
{
	ImageControl* ImageControl::CreateImageControl(string objectName,
		string ImageName, int x, int y)
	{
		...
		if (GameWorld::getInstance().Volkes != NULL)
			GameWorld::getInstance().Volkes->setNewImageControl(_ImageControl);
	...
	ImageControl* ImageControl::CreateImageControl(ImageControl* target)
	{
	...
	ImageControl* ImageControl::CreateImageControl(ImageControl* target, string objectName,
		string ImageName, int x, int y, int width, int height)
	{
		...
		GameWorld::getInstance().Volkes->setNewImageControl(target, _ImageControl);
		return _ImageControl;
	}
	...
	void ImageControl::setIsVerticalReverse(bool isVerticalReverse) {
		IsVerticalReverse = isVerticalReverse;
	}
	void ImageControl::addChild(ImageControl* child)
	{
		m_Child.push_back(child);
	}

	bool ImageControl::isIsVerticalReverse() {
	...
	{
		NotExsistImage = true;
		IsVerticalReverse = false;
		GLfloat _vertexBuffer[15] = {
		...
		for (int i = 0; i < 15; i++)
		{
			vertexBuffer[i] = _vertexBuffer[i];
		}
		uv_x = 0.46875f;
		uv_y = 0.78125f;

		GLfloat _textureBuffer[15] = {
			0.0f ,uv_y ,
			uv_x , uv_y ,
			uv_x , 0.0f ,
			0.0f, 0.0f  ,
		...
		for (int i = 0; i < 15; i++)
		{
			textureBuffer[i] = _textureBuffer[i];
		}
		...
		objtype = GameObejct_TYP_ENUM::MARX_OBJECT_UI;
	...
	ImageControl::ImageControl(string ObjName) : ImageControl()
	{
	...
	ImageControl::ImageControl(string ObjName, float width, float height) : ImageControl()
	{
