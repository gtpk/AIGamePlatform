...
void Md2Object::Refresh()
{
	...
	for (; md2begin != md2End; )
	{
		...
		GameWorld::getInstance().Volkes->setNewPiece(this,node);
		md2begin++;
	}
...
void Md2Object::Load(Md2Object* mother,TiXmlNode * MapPieces)
{
	...
	for (; Piece != NULL; Piece = Piece->NextSibling())
	{
		...
		else if (type == GameObejct_TYP_ENUM::MARX_OBJECT_MD2_OBJECT)
		{
			...
			Md2Object* obj = GameWorld::getInstance().setNewPiece(mother,width, height, TextureName_str.c_str(), AlphaTexture_str.c_str());
			obj->setName(name);
			obj->setRotate(m_rotation[0], m_rotation[1], m_rotation[2]);
			obj->setTranslate(m_translate[0], m_translate[1], m_translate[2]);
			obj->setScale(Scale);
...
void Md2Object::Save(TiXmlElement * MapPieces)
{
	...
	Piece->SetAttribute("GameObejct_TYP_ENUM", this->GetType());
