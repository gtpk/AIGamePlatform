...
SelectableObject::SelectableObject() :GameObejct(MARX_OBJECT_MD2_OBJECT)
{
	SelectNumbers[0] = 0;
	m_bselect = false;
	mother = NULL;
...
SelectableObject::SelectableObject(SelectableObject* _mother) :GameObejct(MARX_OBJECT_MD2_OBJECT)
{
	m_bselect = false;
	mother = _mother;
	_currentName = ObjectNumberingMananger::getInstance()->getNumber(this);
...
SelectableObject::~SelectableObject()
{
	...
	if (node != NULL)
	{
		if (GameWorld::getInstance().Volkes != NULL)
			GameWorld::getInstance().Volkes->DeletePiece(node);

	}
	else
	{
		if (GameWorld::getInstance().Volkes != NULL)
			GameWorld::getInstance().Volkes->DeleteImageControl((ImageControl*)this);
	}
...
SelectableObject * SelectableObject::FindSelectTopObj()
{
	...
	return dynamic_cast<SelectableObject*> (
		ObjectNumberingMananger::getInstance()->GetGameObejct(
...
void SelectableObject::CopyDeepObj(SelectableObject* obj)
{
	...
	for (; md2begin != md2End; )
	{
		...
		if (node != NULL)
		{
			Md2Object* childnode = GameWorld::getInstance().MakePiece(node);
			obj->child.push_back(childnode);
			var->CopyDeepObj(childnode);
		}
		else if (var->GetType() == GameObejct_TYP_ENUM::MARX_OBJECT_UI)
		{
